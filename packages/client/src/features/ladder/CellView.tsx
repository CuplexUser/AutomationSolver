  ...
  return (
    ...
    >
      <svg width={CELL_W} height={CELL_H} viewBox={`0 0 ${CELL_W} ${CELL_H}`}>
        {element ? (
          <>
            <line x1={0} y1={WIRE_Y} x2={26} y2={WIRE_Y} stroke={leftColor} strokeWidth={2} style={glow(leftLive)} />
            <line
              x1={46}
              y1={WIRE_Y}
              x2={CELL_W}
              ...
              stroke={rightColor}
              strokeWidth={2}
              style={glow(rightLive)}
            />
            <Symbol element={element} color={symColor} live={symbolLive} />
            <text x={CELL_W / 2} y={13} textAnchor="middle" className="cell-addr" fill={symColor}>
              {element.device}
            </text>
          ...
          <g className="cell-empty">
            <line x1={CELL_W / 2 - 4} y1={WIRE_Y} x2={CELL_W / 2 + 4} y2={WIRE_Y} stroke="#2b3a4b" strokeWidth={1.5} />
            <line x1={CELL_W / 2} y1={WIRE_Y - 4} x2={CELL_W / 2} y2={WIRE_Y + 4} stroke="#2b3a4b" strokeWidth={1.5} />
          </g>
        )}
      </svg>
    </button>
...
function Symbol({ element, color, live }: { element: LadderElement; color: string; live: boolean }) {
  ...
  if (t === 'contact-no' || t === 'contact-nc' || t === 'contact-rising' || t === 'contact-falling') {
    return (
      <g stroke={color} strokeWidth={sw} style={s} fill="none">
        ...
        {t === 'contact-rising' && <path d="M36 29 l4 6 l-8 0 z" fill={color} stroke="none" />}
        {t === 'contact-falling' && <path d="M36 39 l4 -6 l-8 0 z" fill={color} stroke="none" />}
      </g>
    );
  }
  ...
  if (t === 'coil-out' || t === 'coil-set' || t === 'coil-reset') {
    ...
    return (
      <g stroke={color} strokeWidth={sw} style={s} fill="none">
        ...
        {letter && (
          <text x={36} y={WIRE_Y + 4} textAnchor="middle" fontSize={11} fontWeight={700} fill={color} stroke="none">
            {letter}
          </text>
        )}
      </g>
  ...
  return (
    <g stroke={color} strokeWidth={sw} style={s} fill="none">
      ...
      <text x={36} y={WIRE_Y + 4} textAnchor="middle" fontSize={12} fontWeight={700} fill={color} stroke="none">
        {letter}
      </text>
    </g>
  );
