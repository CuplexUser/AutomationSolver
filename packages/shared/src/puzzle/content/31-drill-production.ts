...
export const drillProduction: PuzzleSpec = {
  ...
  briefing: [
    'Final acceptance test. The infeed now delivers mixed stock: aluminium blanks that',
    'get drilled, and hardened steel blanks that cannot be. An inductive sensor reads',
    'the part on the fixture, and a new diverter routes rejects into the scrap bin.',
    '',
    '## Equipment',
    ...
    '',
    '1. Aluminium (X5 on, X6 off): run the full cycle from the previous work order.',
    '   Clamp, spindle up to speed, feed down, 1.0 s dwell at the bottom, stop the',
    '   spindle, release, then eject with the gate CLOSED so the finished part reaches',
    '   the belt. Light PART DONE (Y3) while it leaves.',
    ...
    '- Feeding into hardened stock snaps the bit and jams the machine.',
    '- All earlier rules still apply: the feed needs CLAMPED (X2) and SPINDLE AT SPEED',
    '  (X7), the pusher is interlocked against the clamp, the beacon (Y2) is lit',
    '  whenever the spindle turns, and the E-Stop drops every actuator.',
    '',
    '## Acceptance',
    '- Three drilled parts on the outfeed belt and every steel blank in the scrap bin,',
    '  with no jam.',
  ...
  hints: [
    ...
    'Y6 is a plain NO contact on M2. The eject coil is now driven by two branches ' +
      '(M0-side eject on M1, or reject on M2), and an OUT coil does not OR across ' +
