...
export const pickPlaceCycle: PuzzleSpec = {
  ...
  briefing: [
    'Commission the robot cell with its first automatic transfer. The articulated arm',
    'serves one infeed pad, fed by the conveyor, and one tray pad. All position sensors',
    'are prewired; your program commands the four axis outputs.',
    '',
    '## Equipment',
    '- X0 AT INFEED and X1 AT TRAY: swing position sensors.',
    '- X10 REACH DOWN and X11 REACH UP: reach axis end sensors.',
    '- X12 GRIPPED: a part is held. X14 SLOT OCCUPIED: the tray pad holds a part.',
    '- Y0 SWING TO TRAY, Y1 SWING TO INFEED, Y2 REACH DOWN, Y3 GRIPPER CLOSE.',
    '- M0: the carrying latch (see Working Registers).',
    '',
    ...
    '3. Close the gripper (Y3). The part is confirmed held when GRIPPED (X12) comes on.',
    '4. Drop Y2 so the arm lifts the part. Keep Y3 on the whole way.',
    '5. At reach up (X11), command Swing to Tray (Y0). Drop Y0 on arrival at the tray',
    '   (X1).',
    '6. Lower the reach again. At X10, drop Y3 to set the part down. The pad confirms',
    '   with SLOT OCCUPIED (X14).',
    '7. Retract, command Swing to Infeed (Y1), and drop Y1 at X0.',
    '',
    '## Interlocks and safety',
    '- The tray pad holds one part. While X14 is on, do not lower the reach or close',
    '  the gripper again. Park the arm retracted at the infeed.',
    '- Only open the gripper while carrying if the reach is fully down. A part released',
    '  mid-air jams the cell and lights the red beacon on the mast.',
    '- The swing axis is interlocked in hardware: the arm refuses to swing while the',
    '  reach is extended.',
  ].join('\n'),
  hints: [
