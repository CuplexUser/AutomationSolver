...
export const packBasics: PuzzleSpec = {
  ...
  briefing: [
    'Welcome to the box packer. A feed belt carries boxes down TWO lanes to an end',
    'stop. The lanes are not in step: sensors BOX NEAR (X14) and BOX FAR (X15) tell',
    'you when each lane\'s box has reached the stop.',
    '',
    ...
    '',
    'Flip BELT RUN (X20) on and make the pusher give every matched pair one clean,',
    'full stroke: extend only once BOTH boxes are at the stop, hold the stroke to the',
    'OUT sensor, then let it spring home for the next pair. Boxes leave the sensors',
    'the moment the plate starts moving, so a bare X14·X15 → Y0 rung lets go',
    'mid-stroke — seal the stroke in and let OUT (X1) break the seal.',
  ].join('\n'),
  hints: [
  ...
  scenarios: [
    {
      ...
      steps: [
        {
          label: 'Belt on — boxes still travelling',
          setInputs: { X20: true },
          holdMs: 400,
        ...
        { label: 'Pair matched — pusher strokes out', holdMs: 400, expect: { Y0: true } },
        {
        ...
        { label: 'Next pair lines up — second stroke', holdMs: 900, expect: { Y0: true } },
        {
    ...
    {
      name: 'Belt off — nothing to push',
      steps: [{ label: 'No boxes arrive, pusher rests', holdMs: 500, expect: { Y0: false, X14: false } }],
    },
  ],
};
