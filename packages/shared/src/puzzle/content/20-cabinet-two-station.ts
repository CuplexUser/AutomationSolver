...
export const cabinetTwoStation: PuzzleSpec = {
  ...
  briefing: [
    'Motor M1 is started and stopped from two places: a local station on the cabinet',
    'door and a remote station at the far end of the line. Either start button has to',
    'run it, and either stop button has to halt it.',
    '',
    '## Equipment',
    '- S1 START and S2 STOP: the local station.',
    '- S3 START and S4 STOP: the remote station.',
    '- K1 CONTACTOR, F1 THERMAL OVERLOAD, M1 MOTOR.',
    '',
    '## Power circuit',
    '1. The usual DOL path: all three phases through the contactor mains, then the',
    '   overload mains, then the motor (L1 to U, L2 to V, L3 to W).',
    '',
    '## Control circuit',
    '1. Put both STOP buttons in series from L1: S2 (21-22), then S4 (21-22). Either',
    '   one breaks the rung.',
    '2. Put both START buttons in parallel after the stops, so either one makes the',
    '   circuit.',
    '3. Add the seal-in auxiliary K1 13-14 in parallel with the whole start group, not',
    '   with one button.',
    '4. Finish through the coil K1.A1-A2 and the overload contact F1 95-96 to N.',
    '',
    '## Field notes',
    '- Stops in series, starts in parallel. That pairing is the whole pattern for',
    '  multi-station control, and it scales to as many stations as you like.',
    '- Sealing across only one start button leaves the other station able to start the',
    '  motor but not to hold it.',
  ].join('\n'),
  hints: [
